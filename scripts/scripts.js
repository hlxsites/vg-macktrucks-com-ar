...
export function decorateLinks(block) {
  [...block.querySelectorAll('a')]
    ...
    .forEach((link) => {
      ...
      const external = !url.host.match('macktrucks.com.ar') && !url.host.match('.hlx.(page|live)') && !url.host.match('localhost');
      if (url.host.match('build.macktrucks.com.ar') || url.pathname.endsWith('.pdf') || external) {
        link.target = '_blank';
      }
    });
}
